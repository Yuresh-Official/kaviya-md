...
const MONGO_URI = process.env.MONGO_URI || 'mongodb+srv://yuresh:yuresh@cluster0.imsvg84.mongodb.net/?appName=Cluster0'; //we need to create a mongodb url soon
...
`;
  ...
  break;
			  ...
			  cmd({
    pattern: "approve",
    desc: "Automatically approve users with a specific country code from the waiting list.",
    react: "✅",
    category: "group",
    filename: __filename
    ...
    try {
        ...
        if (!isBotAdmins) return reply("❌ *Bot Lacks Permission* - Make me an admin.");
        if (!isAdmins) return reply("⚠️ *Permission Denied* - Admin only command.");

        const response = await conn.groupRequestParticipantsList(m.chat);
        if (response.length === 0) return reply("ℹ️ *Waiting list is empty.*");

        const targetCode = config.AUTO_ADD_Country_Code || "94";
        const toAddUsers = response.filter(user => user.jid.startsWith(targetCode));

        if (toAddUsers.length === 0) return reply(`ℹ️ *No users found with code +${targetCode}*`);

        const userJids = toAddUsers.map(user => user.jid);
        await conn.groupRequestParticipantsUpdate(m.chat, userJids, "approve");

        reply(`✅ *Approved ${userJids.length} users (+${targetCode}).*`);
    } catch (e) {
        console.error(e);
...
cmd({
    ...
    try {
        ...
        reply("🚫 *Failed to retrieve requests.*");
    }
})

// --- ACCEPT REQUESTS BY NUMBER ---
cmd({
    pattern: "accept",
    desc: "Accept group join request(s) by index number.",
    react: "✔️",
    category: "group",
    filename: __filename
}, async (conn, mek, m, { from, isGroup, isBotAdmins, reply, q }) => {
    if (!isGroup) return reply("⛔ *Group Only Command*");
    if (!isBotAdmins) return reply("❌ *Bot Lacks Permission*");
    if (!q) return reply("⚠️ *Usage:* .accept 1,2 (Use comma for multiple)");

    try {
        const requests = await conn.groupRequestParticipantsList(from);
        if (requests.length === 0) return reply("ℹ️ *No pending requests.*");

        const indexes = q.split(",").map(num => parseInt(num.trim()) - 1);
        const validJids = indexes.filter(i => i >= 0 && i < requests.length).map(i => requests[i].jid);

        if (validJids.length === 0) return reply("❌ *Invalid request number(s).*");

        await conn.groupRequestParticipantsUpdate(from, validJids, "accept");
        reply(`✅ *Accepted ${validJids.length} join request(s).*`);
    } catch (e) {
...
cmd({
    ...
    try {
        if (!isGroup) return reply("⛔ *Group Only Command*");
        if (!isAdmins) return reply("⚠️ *Admin Only Command*");
        if (!isBotAdmins) return reply("❌ *Bot Lacks Permission*");

        let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : q.replace(/[^0-9]/g, '') + '@s.whatsapp.net';
        if (!users) return reply("⚠️ *Tag a user or reply to a message.*");
...
cmd({
    ...
    if (!users) return reply("⚠️ *Please tag a user.*");
...
cmd({
    ...
    reply("🔒 *Group chat muted (Admins only).*");
})
...
cmd({
    ...
    reply("🔓 *Group chat unmuted (Everyone).*");
})
...
cmd({
    ...
    await reply("👋 *Goodbye! Bot is leaving...*");
...
cmd({
    ...
    if (!isOwner) return;
    if (!q || !q.includes("chat.whatsapp.com")) return reply("❌ *Invalid Link*");

    const code = q.split("chat.whatsapp.com/")[1];
    await conn.groupAcceptInvite(code)
        .then(() => reply("✅ *Joined successfully.*"))
        .catch(() => reply("🚫 *Join failed.*"));
})
 }
  break;
}
